...
import org.mockito.Mockito; // 추가
import org.springframework.http.HttpStatus;
import org.springframework.http.MediaType;
import org.springframework.test.web.servlet.MockMvc; // 추가
import org.springframework.test.web.servlet.assertj.MockMvcTester;
import org.springframework.test.web.servlet.setup.MockMvcBuilders; // 추가
...
class FollowControllerTest {
    ...
    private FollowService followService; // 추가

    private MockMvcTester mvcTester; // 추가

    private final ObjectMapper objectMapper = new ObjectMapper(); // 추가

    @BeforeEach // 추가
    void setUp() {
        followService = Mockito.mock(FollowService.class); // 추가

        FollowController followController = new FollowController(followService); // 추가

        MockMvc mockMvc = MockMvcBuilders.standaloneSetup(followController) // 추가
                .build();

        mvcTester = MockMvcTester.create(mockMvc); // 추가
    }
    ...
    void getFollowees() {
        ...
        FollowListResponse response = new FollowListResponse( // 추가
                List.of(),
                Instant.now(),
                UUID.randomUUID(),
                false,
